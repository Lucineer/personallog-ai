...
## Identity
- **Vessel**: Personallog Ai
- **Role**: PersonalLog.ai
- **URL**: https://personallog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/personallog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/personallog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://personallog-ai.casey-digennaro.workers.dev/health
curl -s https://personallog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=personallog-ai
...
- **vessel.json capabilities**: chat, journal, patterns
- **Fleet link footer**: no
...
1. Keep the vessel small. ~259 lines is the current size.
